...
}// Dump 函数用于将字节数据转换为十六进制字符串并输出到调试窗口
...
int MakeDirectoryInfo() {
    ...
    if (_chdir(strPath.c_str()) != 0) {
        FileInfo finfo;
        memset(&finfo, 0, sizeof(finfo));  // 清零结构体
        finfo.hasNext = FALSE;
    ...
    do {
        FileInfo finfo;
        memset(&finfo, 0, sizeof(finfo));  // 清零结构体，避免垃圾数据
        
        finfo.hasNext = TRUE;
        finfo.isDirectory = (fdata.attrib & _A_SUBDIR) != 0;  // 修复：正确的常量
        
        // 修复：安全的字符串拷贝，包含结束符
        size_t nameLen = strlen(fdata.name);
        if (nameLen >= sizeof(finfo.fileName)) {
            nameLen = sizeof(finfo.fileName) - 1;  // 留出结束符空间
        }
        memcpy(finfo.fileName, fdata.name, nameLen);
        finfo.fileName[nameLen] = '\0';  // 确保字符串结束
        
        printf("fdata.name after memcpy = %s\n", fdata.name);
        TRACE("%s\r\n", fdata.name);
