...
// Add a constructor to the `file_info` struct to handle conversion from BYTE*  
...
#define BFFER_SIZE 4096
	int DealCommand() {
		...
		while (true)
		{
			size_t len = recv(m_client, szBuffer+index, BFFER_SIZE - index, 0); // 接收数据
			int temp = len;
			if (temp <= 0) {
				delete[]szBuffer;
				return -1; // 连接断开或出错
	...
	bool GetFilePath(std::string& strPath) {
		if ((m_packet.sCmd >=2) && (m_packet.sCmd <=9)) {
	...
	void CloseClient() {
		if (m_client != INVALID_SOCKET) {
			closesocket(m_client); // 关闭客户端套接字
			m_client = INVALID_SOCKET; // 重置客户端套接字为无效值
		}
	}
